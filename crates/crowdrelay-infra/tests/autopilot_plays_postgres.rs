...
use crowdrelay_application::autopilot::{
    ...
    ClaimedAutopilotAction, PlayAudience, PlayStart, PlayStepPlan, PlayStepSettlement,
};
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let anchor = anchors
        .iter()
        .find(|anchor| anchor.event_id == event_id)
    ...
    assert!(
        !repository
            ...
            .await?
            .iter()
            .any(|anchor| anchor.event_id == event_id),
    ...
    let play = snapshots
        .iter()
        .find(|play| play.event_id == event_id)
    ...
    assert!(
        repository
            ...
            .await?
            .iter()
            .all(|play| play.event_id != event_id),
