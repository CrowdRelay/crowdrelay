...
/// returned unchanged.
///
/// The `ON CONFLICT DO UPDATE SET designed_at = EXCLUDED.designed_at` is a
/// no-op update that exists only to trigger `RETURNING` on the conflict
/// path — Postgres does not return the existing row from a plain
/// `DO NOTHING`. The timestamp update is harmless and records the last
/// access time.
...
pub(in crate::autopilot) async fn get_or_create_experiment_design(
    ...
    // Upsert: on conflict, update designed_at (no-op touch) so RETURNING
    // gives us the existing row. The experiment_uuid is generated by
    // Postgres (DEFAULT gen_random_uuid()) on the INSERT path.
    let row = sqlx::query(
        r#"
        ...
        DO UPDATE SET designed_at = EXCLUDED.designed_at
    ...
    // P0-4: Run the power check on the design and persist the results.
    // The power check computes expected_treatment/control counts and
    // determines whether the experiment has enough power for a meaningful
    // randomized holdout. We persist the results so the DB row reflects
    // the actual status, not just the default 'active'.
