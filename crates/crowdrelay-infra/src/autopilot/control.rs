                ...
                match attribution {
                    ChannelAttribution::Unattributed { reason } => {
                        ...
                        if let Some(existing) =
                            unattributed.iter_mut().find(|group| group.reason == reason)
                        {
                            existing.signups = existing.signups.saturating_add(signups);
                            existing.activated_30d =
                                existing.activated_30d.saturating_add(activated);
                        } else {
                    ...
                    attributed => channels.push(ChannelPerformance {
                        ...
                        activation_basis_points: (signups > 0).then(|| {
                            u32::try_from(
                                u64::from(activated).saturating_mul(10_000)
                                    / u64::from(signups),
                            )
                            .unwrap_or(u32::MAX)
                        }),
