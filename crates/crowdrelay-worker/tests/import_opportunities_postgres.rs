...
Monitorowac otwarcie rundy,Polski nabor metalowy,Zweryfikowane 2026"
    )?;
    // Reachable only by submission form. Still a route, so it lands.
    writeln!(
        file,
        "ZG-002,review_contest,Road to Mystic,Road to Mystic,,\
https://example.test/apply,PL,Krakow,9500,2500,,true,9,Niska,A,metal,,,Historyczny"
    )?;
...
PL,Lodz,7000,4000,,false,5,Brak,B,,,,"
    )?;
    // Refused: no address and no form. Nothing could act on it.
    writeln!(
        file,
        "ZG-004,festival,No Route Festival,No Route Festival,,,PL,Gdansk,9000,2500,,true,7,Niska,C,,,,"
    )?;
    // Refused: a kind the table's CHECK does not accept.
    writeln!(
        file,
        "ZG-005,juwenalia,Bad Kind Fest,Bad Kind Fest,x@example.test,,PL,Poznan,8000,2500,,true,5,Niska,C,,,,"
    )?;
    file.flush()?;
    Ok(file)
}
...
async fn reimport_inner(pool: &PgPool) -> Result<()> {
    ...
    writeln!(
        second,
        "ZG-001,festival,Wacken Metal Battle POLSKA,Wacken Metal Battle POLSKA,\
newaddress@example.test,,PL,Warszawa,9700,7000,2026-03-20,true,3,Wysoka,A,metal,,,Zweryfikowane 2026"
    )?;
    writeln!(
        second,
        "ZG-003,showcase,Targi Closed 2026,Targi Closed 2026,book@example.test,,\
PL,Lodz,7000,4000,,true,5,Brak,B,,,,"
    )?;
    second.flush()?;
