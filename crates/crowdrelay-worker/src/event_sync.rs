...
mod tests {
    ...
    fn bandsintown_source() -> EventSourceRow {
        EventSourceRow {
            id: Uuid::nil(),
            workspace_id: Uuid::nil(),
            provider: "bandsintown".to_owned(),
            artist_name: "VIRYA".to_owned(),
            app_id: "test-app".to_owned(),
            default_country_code: "PL".to_owned(),
    ...
    fn bandsintown_event(id: &str, datetime: &str) -> BandsintownEvent {
        BandsintownEvent {
            id: serde_json::Value::String(id.to_owned()),
            url: None,
            datetime: datetime.to_owned(),
            title: Some(format!("Gig {id}")),
            description: None,
            lineup: None,
            venue: BandsintownVenue {
                name: Some("Klub".to_owned()),
                city: Some("Warszawa".to_owned()),
                region: None,
                country: Some("Poland".to_owned()),
                location: None,
                latitude: None,
                longitude: None,
            },
    ...
    #[test]
    fn unparseable_event_is_skipped_without_disarming_the_rest() {
        let source = bandsintown_source();
        let payload = vec![
            bandsintown_event("1", "2026-09-11T20:00:00"),
            // A DST-fold timestamp: real, but not resolvable to one instant.
            bandsintown_event("2", "2026-10-25T02:30:00"),
            bandsintown_event("3", "2026-09-12T20:00:00"),
        ];
        ...
        for event in payload {
            ...
            match normalize_bandsintown_event(event, &source) {
                ...
                Err(_) => skipped.push(raw_id),
            }
        }
        let batch = ProviderBatch { events, skipped };

        // The good gigs survive instead of the whole calendar being dropped...
        assert_eq!(batch.events.len(), 2);
        assert_eq!(batch.skipped, vec!["2".to_owned()]);
        // ...and the batch reports itself incomplete, which is what keeps
        // persist_success from cancelling the event it could not read.
    ...
    fn fully_parseable_batch_stays_authoritative() {
        ...
        let batch = ProviderBatch {
            events: vec![
                normalize_bandsintown_event(bandsintown_event("1", "2026-09-11T20:00:00"), &source)
