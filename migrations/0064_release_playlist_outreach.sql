...
--
-- `start_press` is the durable fact that a release entered its outbound pitching
-- window. Press/radio opportunities are seeded in Rust today; this additive
-- trigger keeps playlist targets in the same atomic release transaction without
-- inventing recipients or bypassing target verification. The ON CONFLICT path
-- makes it safe to remove this compatibility trigger later when the Rust seeder
-- grows native playlist coverage.
