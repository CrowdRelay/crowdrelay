    ...
    function testRejectsRootMutationAndUnauthorizedCaller() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ViryaProofAnchor.BatchCommitmentConflict.selector, batch, commitmentA, commitmentRootB
            )
        );
        vm.prank(signer);
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ViryaProofAnchor.BatchCommitmentConflict.selector, batch, commitmentA, commitmentCount2
            )
        );
        vm.prank(signer);
