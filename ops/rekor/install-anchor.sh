...
# `crowdrelay-api-active` is the blue-green alias and the value this should
# normally hold; the bare `crowdrelay-api` is only published by
# compose.production.yaml and stops resolving under blue-green. Both stay
# accepted because both are private Docker endpoints, which is what this guard
# is actually for — it exists to reject public Caddy/HTTPS URLs.
grep -Eq '^CROWDRELAY_INTERNAL_URL=http://(crowdrelay-api-active|crowdrelay-api-green|crowdrelay-api|api):8080/?$' "$env_file" \
